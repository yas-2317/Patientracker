...
import { phq9Data, phq9ItemLabels, PHQ9Item } from '@/data/assessments';

// Phase boundary dates
const BEFORE_END = '2024-02-15';
const DURING_END = '2024-05-01';
...
// KPI data (dummy values as specified)
const kpiData = [
...
// Classify data points into phases
...
// Calculate average total score for each phase
function calcAvg(data: typeof phq9Data): number {
...
// Bar chart data for each phase (single bar each)
const beforeBarData = [{ name: '治療前', score: beforeAvg }];
const duringBarData = [{ name: '治療中', score: duringAvg }];
const afterBarData = [{ name: '治療後', score: afterAvg }];

// Line chart: all data points with phase info
const lineChartData = phq9Data.map((d) => {
  const phase =
    d.date <= BEFORE_END ? 'before' : d.date <= DURING_END ? 'during' : 'after';
  return {
    date: d.date,
    score: d.total,
    phase,
...
// Get ReferenceArea boundaries from data dates
const allDates = phq9Data.map((d) => d.date);
const firstDate = allDates[0];
const lastDate = allDates[allDates.length - 1];

// Radar chart data: average each PHQ-9 item per phase
const itemKeys = Object.keys(phq9ItemLabels) as (keyof PHQ9Item)[];

function calcItemAvg(data: typeof phq9Data, key: keyof PHQ9Item): number {
  if (data.length === 0) return 0;
  const sum = data.reduce((acc, d) => acc + d.items[key], 0);
  return Math.round((sum / data.length) * 10) / 10;
}
...
const phaseColumns = [
  {
    ...
    color: PHASE_COLORS.before,
    avg: beforeAvg,
    barData: beforeBarData,
    count: beforeData.length,
  },
  {
    id: 'during',
    label: '治療中',
    period: '2024-02-15 〜 2024-05-01',
    color: PHASE_COLORS.during,
    avg: duringAvg,
    barData: duringBarData,
    count: duringData.length,
  },
  {
    id: 'after',
    label: '治療後',
    period: '2024-05-01 〜',
    color: PHASE_COLORS.after,
    avg: afterAvg,
    barData: afterBarData,
    count: afterData.length,
  },
];
...
function LineTooltip({ active, payload, label }: {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg px-3 py-2 shadow-sm text-sm">
      ...
      <p style={{ color: phaseColor }} className="font-semibold">
        {phaseLabel}
      </p>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {/* Radar chart: Before vs After per PHQ-9 item */}
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
          <h2 className="text-base font-semibold text-slate-800 mb-1">
            PHQ-9 項目別比較（Before vs After）
          </h2>
          <p className="text-xs text-slate-400 mb-4">
            9項目の平均スコアをフェーズ間で比較
          </p>
